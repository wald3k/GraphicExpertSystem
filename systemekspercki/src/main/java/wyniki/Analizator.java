...
/**
 *Generalnie przyjmuje Wyniki, Quiz, oraz kryteria i na podstawie tego tworzy tabelkê;
 * Potem ma za zadanie wybraæ z bazy programy najbardziej zbli¿one do wygenerowanej tabelki.
 * @author Freeman
 */
public class Analizator {
    ...
    public void policzWynikiDlaXPytania(int x){
        ...
        for (Ranking r : queryKat.getResultList()) { 
            ...
            if(tempPytanie.getZaznaczonaOdpowiedz().getTresc().equals("tak")){//liczy punkty tylko dla tak. Jest to najprostsza wersja                
                double starePunkty = tempWynik.getWynik();
                double wagaPytania  = tempPytanie.getWagaPytania();
                double punktyRankingowe = r.getPunkty();
                tempWynik.setWynik(starePunkty + wagaPytania * punktyRankingowe);                       
            }  
            licznik++; 
        }
